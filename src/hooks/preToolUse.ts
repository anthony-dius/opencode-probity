...
import { loadOpenCodePluginConfig } from '../config/opencode.ts';
...
export function createProbityHook(options?: ProbityHookOptions) {
  ...
  const projectConfig = loadOpenCodePluginConfig(projectDir);
  const configPath = options?.configPath ?? projectConfig.configPath;
  const debugPath = options?.debugPath ?? projectConfig.debugPath;
  ...
  return async (input: ToolInput, output: ToolOutput): Promise<void> => {
    ...
    try {
      if (!adapter) {
        const resolvedConfigPath = configPath ?? (await findProbityConfig(projectDir));
